...
#     # Resample region_forecast if necessary
#     if freq == "15T":
#         region_forecast = region_forecast.set_index("ds").resample("15T").interpolate().reset_index()
...
#         # Fraction of daily total per period
...
#         # Total daily energy from state forecast (in MU)
#         energy_mu = state_forecast[state_forecast["date"] == d]["yhat"].values[0]

#         # Convert MU to MW per period: MW = MU * 1000 / hours_in_day
#         hours_in_day = 24 if freq == "H" else 24 * 4  # 15-min => 96 periods
#         r["mw"] = r["fraction"] * energy_mu * 1000 / hours_in_day
...
#                 "datetime": row["ds"],
#                 "mw": float(row["mw"]),
#                 "temperature": float(row["temperature_c"]) if "temperature_c" in row else None
#             })
